 ...
 */
const loadView = (view) => {
  return (resolve) => require([`@/views/${view}`], resolve)// 开发环境下 webpack4中动态import不支持变量方式
}
...
const filterAsyncRouter = (routers) => { // 遍历后台传来的路由字符串，转换为组件对象
  return routers.filter(router => {
    ...
    return true // 返回true表示保留该元素（通过测试），false则不保留。
  })
}
      ...
      children: [
        {
          ...
          name: 'Test',
          meta: { 'title': '测试', 'icon': 'permission', 'noCache': true }
        }
      ]
    }
  ]
